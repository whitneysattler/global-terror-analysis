...
summary(factor(gdtCleanedRemoved$vicinity))
View(gdtCleanedRemoved[which(gdtCleanedRemoved$vicinity == -9),])
# There are two observations that can have vicnity imputed based on other data
gdtCleanedRemoved[which(gdtCleanedRemoved$vicinity == -9),]$vicinity = c(1, -9, 1, -9, -9)
# The since we don't have a way to impute the vicinity variable for the remaining three, we will leave these as -9.

#crit1, crit2, crit3
summary(factor(gdtCleanedRemoved$crit1))
summary(factor(gdtCleanedRemoved$crit2))
summary(factor(gdtCleanedRemoved$crit3))
...
gdtCleanedRemoved$alternative[which(is.na(gdtCleanedRemoved$alternative))] = 6

# int_LOG, INT_IDEO, INT_MISC, INT_ANY
summary(factor(gdtCleanedRemoved$INT_LOG))
summary(factor(gdtCleanedRemoved$INT_IDEO))
...
summary(factor(gdtCleanedRemoved$INT_MISC))
...
gdtCleanedRemoved$INT_MISC[which(gdtCleanedRemoved$INT_MISC == -9)] = NA
...
gdtCleanedRemoved = gdtCleanedRemoved[ , !(names(gdtCleanedRemoved) %in% c("nreleased"))]

# nHours, nDays
# We decided to measure the length of all events in hours.  Therefore, we transform and elminiate the nDays variable
View(gdtCleanedRemoved[, names(gdtCleanedRemoved) %in% c("nhours", "ndays")])
gdtCleanedRemoved$nhours[which(gdtCleanedRemoved$ndays > 0)] = gdtCleanedRemoved$ndays[which(gdtCleanedRemoved$ndays > 0)] * 24
...
gdtCleanedRemoved$nhours[which(gdtCleanedRemoved$nhours < 0)] = NA

# success
summary(factor(gdtCleanedRemoved$success))
...
gdtCleanedRemoved = gdtCleanedRemoved[ , !(names(gdtCleanedRemoved) %in% c("success"))]

# suicide
summary(factor(gdtCleanedRemoved$suicide))
...
gdtCleanedRemoved = gdtCleanedRemoved[ , !(names(gdtCleanedRemoved) %in% c("suicide"))]

# gname, gname2, gname3, gsubname, gsubname2, gsubname3
summary(factor(gdtCleanedRemoved$gname))
...
gdtCleanedRemoved = gdtCleanedRemoved[ , !(names(gdtCleanedRemoved) %in% c("gsubname", "gname2", "gsubname2", "gname3", "gsubname3"))]

# Divert
summary(factor(gdtCleanedRemoved$divert))
# This variable is degenerative. 8,123 cases are unknown
gdtCleanedRemoved = gdtCleanedRemoved[ , !(names(gdtCleanedRemoved) %in% c("divert"))]

# multiple
summary(factor(gdtCleanedRemoved$multiple))
...
gdtCleanedRemoved = gdtCleanedRemoved[ , !(names(gdtCleanedRemoved) %in% c("targsubtype1", "corp1", "target1", "targtype2", "targsubtype2", "corp2", "target2", "natlty2", "targtype3", "targsubtype3", "corp3", "target3", "natlty3", "natlty3_txt", "guncertain2", "guncertain3"))]

#Nationalaity Type converted from country to region. Keeping 999 for unknown nationality region
library(sqldf)
regions = sqldf("select distinct country, region from gdtCleanedRemoved")
gdtCleanedRemoved = sqldf("select a.*,
                            case
                              when a.natlty1 = 999 then 999
                              else b.region 
                            end as natltyregion
                          from gdtCleanedRemoved as a
                          left join regions as b
                            on a.natlty1 = b.country")

gdtCleanedRemoved = gdtCleanedRemoved[ , !(names(gdtCleanedRemoved) %in% c("natlty1"))]
```
