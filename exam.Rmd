...
#test = gtdCleanedRemoved[, !(names(gtdCleanedRemoved) %in% c("ransomamt", "ransompaid", "percPaid"))]
test = gtdCleanedRemoved

test$response = factor(ifelse(test$hostkidoutcome %in% c(2,5), 1, 0))
test = test[, !(names(test) %in% c("hostkidoutcome"))]

glm(response~ ., data = test, family = "binomial")
