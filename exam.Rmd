...
# percPaid
gtdCleanedRemoved$percPaid = gtdCleanedRemoved$ransompaid / gtdCleanedRemoved$ransomamt
...
```{r}
#Russell's code for SVM

#Remove gname
gtdCleanedRemoved = gtdCleanedRemoved[, !(names(gtdCleanedRemoved) %in% c("gname"))]

#Check which variables have missing values
sapply(gtdCleanedRemoved, function(x) sum(is.na(x)))

#nhostkid imputation
summary(gtdCleanedRemoved$nhostkid)

train = gtdCleanedRemoved[, !(names(gtdCleanedRemoved) %in% c("nhours","ransomamt","ransompaid","percPaid","response"))]

preProcvalues = preProcess(train, method = c("bagImpute"))
nhostkidImpute = predict(preProcvalues, train)

summary(nhostkidImpute$nhostkid)

train$nhostkid = nhostkidImpute$nhostkid

#nhours imputation
train$nhours = gtdCleanedRemoved$nhours

preProcvalues = preProcess(train, method = c("bagImpute"))
nhoursImpute = predict(preProcvalues, train)

summary(nhoursImpute$nhours)
summary(gtdCleanedRemoved$nhours)

train$nhours = nhoursImpute$nhours

#ransomamt imputation
train$ransomamt = gtdCleanedRemoved$ransomamt
train$ransomamt[which(train$ransomamt < 0)] = NA

preProcvalues = preProcess(train, method = c("bagImpute"))
ransomamtImpute = predict(preProcvalues, train)

summary(ransomamtImpute$ransomamt)
summary(gtdCleanedRemoved$ransomamt)

train$ransomamt = ransomamtImpute$ransomamt

#ransompaid imputation
train$ransompaid = gtdCleanedRemoved$ransompaid
train$ransompaid[which(train$ransompaid < 0)] = NA

preProcvalues = preProcess(train, method = c("bagImpute"))
ransompaidtImpute = predict(preProcvalues, train)

summary(ransompaidtImpute$ransompaid)
summary(gtdCleanedRemoved$ransompaid)

train$ransompaid = ransompaidtImpute$ransompaid

#Recalculate percPaid for imputed ransoms. Need to get rid of 0's in ransomamt
# train$percPaid = train$ransompaid / train$ransomamt
# 
# summary(train$percPaid)

#SVM
train$response = gtdCleanedRemoved$response
train = train[,!(names(train) %in% c("country"))]
levels(train$response) <- c("noDeaths", "Deaths")
train = train[, !(names(train) %in% c("targtype1"))]
...
trainSVM =  train[train_ind, ]
testSVM = train[-train_ind, ]

set.seed(1) 
svmFit <- train(response ~ .,  data = trainSVM,
                method = "svmRadial", 
